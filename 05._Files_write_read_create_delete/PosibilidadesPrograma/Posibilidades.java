...
//Los dos primeros import trabajan con funciones que he creado yo, el resto son para el tratamiento de files, escritura, lectura, etc. Scanner y Excepcion Input output
...
public class Posibilidades {
    ...
    public static void carpetaArchivos() {      //Función que crea carpeta 
        File carpeta = new File(rutaCarpetaArchivos());
        if (carpeta.mkdir() == true) {
            System.out.println("Carpeta -Archivos- creada con éxito");  //En caso de que no existiera antes muestra por pantalla mensaje, si no no muestra nada
        }
            ...
            try {
                ...
                if (listado.length > 0) { // Si la longitud del array es mayor que 0, es decir, si hay archivos en la carpeta archivos lo siguiente
                    ...
                    if (num < 1 || num > listado.length) { // Si el número no está entre 1 y la longitud del array, la opción no es válida
                        ...
                        ok = true;      
                        /*Lo pongo porque si no ponía nada al ejecutar el programa despues de mostrarme mensaje anterior esperaba 
                        a que introduciera algun dato antes de volver a mostrar listado*/
                    ...
                    ok= false;      //Para salir del bucle y por tanto de la opción menú
                }
                ...
                System.out.println("La opción elegida no es válida.");
                System.out.println("El número tiene que estar comprendido entre las opciones señaladas:");
                sc.next(); // Siguiente buffer para que no se quede hasta el infinito
            }
        }
    }
        ...
        while (ok) {
            ...
            if (listado.length > 0) {
                ...
                try {
                    ...
                    sc.next();      //Siguiente bufer para que no se quede hasta el infinito
                }
